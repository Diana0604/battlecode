...
public class Soldier extends RobotPlayer {
	...
	private static int[][] eSoldier = {{-1000000, -2100, -2000, -1000, 20, 10, 0},{-1000000, -2100, -2000, -1500, -1250, -1000, 0}};
	private static int[] sZombie = {-1000000, -1500, -1250, -500, 20, 10, 0};
	
	private static int torns_combat = 5;
	
	private static int taxista (MapLocation a, MapLocation b) {
		return Math.max(Math.abs(a.x-b.x),Math.abs(a.y-b.y));
	}
	
	static private int enCombat = 0;
	static private MapLocation ls = null; //location del signal
            ...
            try {
    			...
    			for (int i = 0; i < sig.length; ++i) {
    				...
    				if (sig[i].getMessage() == null) {
    					...
    					// no es senyal doble
    					if (enCombat == 0 && ls == null) ls = sig[i].getLocation();
    				}
    				else {
    					// senyal guai
    				}
    			...
    			if (nenemies+nzombies > 0) {
        			ls = null;
        			if (enCombat == 0) rc.broadcastSignal(visionRange);
