...
public class Archon extends RobotPlayer{
	...
	private static MapLocation loc, posicio_bruta;
	...
	private static void enviarClearRubble() throws GameActionException {
		...
		int x = posicio_bruta.x;
		int y = posicio_bruta.y;
	...
	private static void enviarSignalStage3() throws GameActionException {
    	int mode = Message.STAGE3;
		int object = Message.NONE;
		int typeControl = Message.NONE;
		int robotType = Message.NONE;
		int x = targetLocation.x;
	        	 ...
	        	 // Ordre d'importancia:
            		...
            		if (rc.isCoreReady()) {
            			if (rc.hasBuildRequirements(RobotType.TURRET)) buildTurtle();
	        				...
	        				if (rc.canMove(d)) {
	        					rc.move(d);
	        					rc.setIndicatorString(1, "Vaig directe");
	        				}
	        				else if (rc.canMove(d.rotateLeft())) {
	        					rc.move(d.rotateLeft());
	        					rc.setIndicatorString(1, "Vaig esquerra");
	        				}
	        				else if (rc.canMove(d.rotateRight())) {
	        					rc.move(d.rotateRight());
	        					rc.setIndicatorString(1, "Vaig dreta");
	        				}
	        			}
	                }
        			enviarGoTurtle();
